...
(* 
  Changes by recursive rebels: 
  Type typL added
  freeTypeVars added TypL 
  typeToString added TypL
  showType added TypL
  unify added TypL 
  copyType added TypL
*)
...
type typ =
     | TypI                                (* integers                   *)
     | TypB                                (* booleans                   *)
     | TypL of typ                         (* List of elements of type typ*)
...
let rec unify t1 t2 : unit =
    ...
    | (TypL t1, TypL t2) when t1 <> t2 -> failwith ("type error: lists are of " + typeToString t1 + " and " + typeToString t2)
